...
- T4 (`aria query --json` CLI smoke): Added `test_query_json_placeholder_returns_valid_payload` in `tests/unit/test_cli_entry.py` — invokes `aria query "test question" --json` with `ARIA_PLACEHOLDER_API=true` via CliRunner env and asserts exit 0, valid JSON, all five `_success_payload` keys, and `aria_mode == "placeholder"`. Rationale: post–G8-flip default, explicit env isolation is required to exercise the `--json` path without live backends. **Coverage gap (deferred):** no unit test for `aria query --json` on `service_unavailable` (exit 1, alternate JSON body).
...
- T6 (Golden path wet run): Executed Phase 1 live session with `ARIA_PLACEHOLDER_API=false`; filled `.dev/MVP_PICKUP.md` wet run log and `.dev/decision-logs/T6-wet-run.md`. All CLI steps exit 0 on Neo4j + Chroma + OpenAI `gpt-4o-mini`; confirmed ingest preflight requires LLM while `GET /ready` returns 200 with `llm:false`. No code fixes. **Coverage gap (deferred):** no CI job replays the wet-run script; Chroma compose healthcheck false negative and impact-chain emptiness documented in decision log.
- T5 (Status preflight clarification): `aria status` human-readable output now ends with a note that LLM health is optional for status exit code 0 but required for `aria ingest`. + added `tests/unit/test_status.py`.
...
- T2 (pytest integration markers): Applied `@pytest.mark.integration` to all test classes in `tests/integration/test_end_to_end.py` and `tests/integration/test_ingestion_pipeline.py`, and updated the `pyproject.toml` marker description so Phase 1’s `pytest tests/integration -m integration` collects and runs 25 tests without live infra. Resolves context-map Flag 1 (marker/command mismatch). **CI note:** workflows using `pytest -m "not integration"` now exclude these mocked integration tests from dry runs, reducing non-live coverage silently—CI workflow changes remain out of scope. **Coverage gap (deferred):** no automated check that new classes under `tests/integration/` receive the marker.
- T3 (Fix aria serve port default): `aria serve` now defaults to port **8080** and honors **`API_PORT`** at invocation time (Pattern B), avoiding conflict with Chroma on host port **8000**. Rationale: docker-compose binds Chroma to 8000; the previous serve default collided. + added `tests/unit/test_serve.py`. Deferred adversarial test: non-integer **`API_PORT`** raising **`ValueError`** at startup (operator misconfiguration; out of Phase 1 scope).
- T4 (Surface regulation IDs post-ingest): After a successful graph write, `aria ingest` prints `regulation_ids:` from Neo4j (`MATCH (r:Regulation) RETURN r.id AS id`) so the wet run can pass a real id to `aria impact`. Rationale: resolves context-map Flag 3 (no documented discovery path). + added `tests/unit/test_ingest_command.py`. Deferred adversarial test: Neo4j `execute_read` failure during ID fetch (live error path; T6 wet run).
